...
class DemoAgent:
    def __init__(
        ...
        **params,
        ...
        self.wallet_name = (
            params.get("wallet_name") or self.ident.lower().replace(
                " ", "") + rand_name
        )
    ...
    def get_agent_args(self):
        ...
        if self.genesis_txn_list:
            result.append(
                ("--genesis-transactions-list", self.genesis_txn_list))
        if self.seed:
    ...
    async def register_or_switch_wallet(
        ...
        if public_did:
            if cred_type == CRED_FORMAT_INDY:
                ...
                data = {"method": DID_METHOD_KEY,
                        "options": {"key_type": KEY_TYPE_BLS}}
    ...
    async def service_decorator(self):
        ...
        endpoint_url = (
            "/wallet/get-did-endpoint" + "?did=" +
            agent_public_did["result"]["did"]
        )
    ...
    async def agency_admin_GET(
        ...
        if not self.multitenant:
            raise Exception(
                "Error can't call agency admin unless in multitenant mode")
        try:
    ...
    async def admin_GET_FILE(self, path, params=None, headers=None) -> bytes:
        try:
            ...
            params = {k: v for (k, v) in (
                params or {}).items() if v is not None}
    ...
    async def collect_postgres_stats(self, ident: str, vacuum_full: bool = True):
        ...
        for row in sizes:
            if row["relation"] in results:
                results[row["relation"]
                        ][1] = row["total_size"].replace(" ", "")
        for t in tables:
